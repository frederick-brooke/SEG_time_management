...
describe("LoginPage", () => {
	...
	it("shows invalid credentials error", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Initiate Launch" }),
		);
		await waitFor(() => {
			expect(
	...
	it("shows banned error and BannedPage component", async () => {
		...
		fireEvent.submit(
			screen
				.getByRole("button", { name: "Initiate Launch" })
				.closest("form")!,
		);
		await waitFor(() => {
			expect(
				screen.getByText("Your account has been banned."),
			).toBeInTheDocument();
	...
	it("shows error if session lacks user id", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Failed to get user session."),
			).toBeInTheDocument();
		});
	});
