...
export default function ModulesPageClient({
	...
	const sorted = useMemo(
		() => sortModules(initialModules, sortKey),
		[initialModules, sortKey],
	);
				...
				<div className="flex flex-col items-center sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
					<div className="text-center sm:text-left">
						...
						<p className="lunar-page-subtitle">
							Collaborate with peers on shared goals
						</p>
					</div>
					<div className="flex gap-3">
						<Button
							onClick={() => setShowJoin(true)}
							className="px-6 py-3 rounded-2xl bg-white/5 ring-1 ring-white/10 text-white/80 font-medium hover:bg-white/10 transition flex items-center gap-2"
						>
							<LogIn size={16} /> Join
						</Button>
						<Button
							onClick={() => setShowCreate(true)}
							className="px-6 py-3 rounded-2xl bg-blue-300 text-gray-950 font-semibold shadow-[0_0_30px_rgba(90,150,255,0.25)] hover:shadow-[0_0_50px_rgba(90,150,255,0.45)] transition flex items-center gap-2"
						>
							<Plus size={16} /> Create
						</Button>
					</div>
				...
				<div className="flex justify-between items-center mb-6 bg-white/5 p-4 rounded-xl border border-white/10">
					...
					<div className="relative">
						<Button
							...
							className="px-6 py-3 rounded-2xl bg-white/5 ring-1 ring-white/10 text-white/80 font-medium hover:bg-white/10 transition text-xs flex items-center gap-2"
						>
