...
function useShopManager(initialData: ShopData) {
  ...
  const executeAction = useCallback(
    (actionFn: () => Promise<void>, successMsg: string) => {
  ...
  const buy = useCallback(
    (id: string) => {
      ...
      executeAction(async () => {
        ...
        setData((p) => ({
          ...p,
          points: p.points - item.price,
          items: p.items.map((i) => (i.id === id ? { ...i, owned: true } : i)),
        }));
      }, `🎉 ${item.name} purchased!`);
    },
  ...
  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-gray-900/80 backdrop-blur border border-white/10 rounded-3xl p-8">
    <div>
      ...
      <p className="text-white/40 text-sm max-w-md mt-2">
        Unlock space-themed profile pictures and equip your favourite identity.
      </p>
    </div>
...
const AvatarThumbnail = ({
  ...
  <div
    onClick={onClick}
    title={item.name}
    className={`relative w-14 h-14 rounded-full overflow-hidden ring-2 cursor-pointer transition-transform hover:scale-110 ${isEquipped ? "ring-yellow-400" : "ring-white/20"}`}
  >
    <img
      src={AVATAR_IMAGES[item.value as keyof typeof AVATAR_IMAGES]}
      alt={item.name}
    ...
    {isEquipped && (
      <div className="absolute inset-0 bg-yellow-400/20 flex items-end justify-center pb-1">
        <span className="text-[8px] font-black text-yellow-900 bg-yellow-300 px-1 rounded">
          ON
        </span>
      </div>
    )}
...
export default function ShopPageClient({
  ...
  const { data, isPending, toast, buy, equip, unequip } =
    useShopManager(initialData);
  ...
  return (
    <LunarThemeWrapper>
      <div className="flex flex-1 flex-col p-6 pt-0">
              ...
              <div className="flex gap-3 flex-wrap">
                {ownedAvatars.map((item) => (
                  <AvatarThumbnail
                    key={item.id}
                    item={item}
                    isEquipped={data.equippedAvatar === item.value}
                    onClick={() =>
        ...
        {toast && (
          <div
            className={`fixed bottom-6 right-6 z-50 px-5 py-3 rounded-2xl shadow-xl font-bold text-sm ${toast.type === "success" ? "bg-gray-900 text-white" : "bg-red-500 text-white"}`}
          >
            {toast.message}
          </div>
        )}
