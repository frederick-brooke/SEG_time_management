...
jest.mock("@/context/UIContext", () => ({
	useUI: () => ({
		wellbeingOpen: mockWellbeingOpen,
		setWellbeingOpen: setWellbeingOpenMock,
	}),
}));
...
jest.mock("@/app/actions/profile", () => ({
	getMyProfile: (...a: any[]) => getMyProfileMock(...a),
}));
jest.mock("@/hooks/useTasks", () => ({
	useTasks: (...a: any[]) => useTasksMock(...a),
}));
...
jest.mock("../wellbeing/page", () => () => <div>WellbeingPage</div>, {
	virtual: true,
});
...
jest.mock("@/components/ui/RocketProgress", () => ({
	RocketProgress: ({ progress }: any) => (
		<div data-testid="rocket">Rocket {progress}%</div>
	),
}));
...
describe("Dashboard Page", () => {
	...
	it("shows 0% when progress context returns 0", async () => {
		(useTaskProgress as jest.Mock).mockReturnValue(
			makeProgressContext({ progressPercentage: 0 }),
		);
		render(<Page />);
		expect(await screen.findByTestId("rocket")).toHaveTextContent(
			"Rocket 0%",
		);
	});
	...
	it("renders the wellbeing button", async () => {
		render(<Page />);
		expect(
			await screen.findByLabelText("Open wellbeing panel"),
		).toBeInTheDocument();
	});
	...
	it("calls setWellbeingOpen(true) and hides button on click", async () => {
		...
		expect(
			screen.queryByLabelText("Open wellbeing panel"),
	...
	it("does not fetch dashboard data while session is loading", async () => {
		setAuth("loading");
		...
		expect(refreshProgressMock).not.toHaveBeenCalled();
	});

	it("falls back to empty leaderboard when leaderboard data is null", async () => {
		(getFriendsLeaderboard as jest.Mock).mockResolvedValueOnce(null);

		render(<Page />);

		expect(await screen.findByText(/Welcome, Test/i)).toBeInTheDocument();
		await waitFor(() => {
			expect(getFriendsLeaderboard).toHaveBeenCalledWith("week");
		});
	});

	it("refetches exams when session user exists", async () => {
		render(<Page />);
