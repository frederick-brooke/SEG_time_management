...
function RankDisplay({ rank }: { rank: number }) {
  ...
  return <span className="lunar-label !mb-0 opacity-50 tabular-nums">{rank}</span>;
}
...
function LeaderboardRow({ user, rank }: { user: LeaderboardUser; rank: number }) {
  ...
  return (
    <div className={`flex items-center px-3 sm:px-5 py-3 sm:py-4 hover:bg-white/[0.02] transition-colors gap-2 ${user.isCurrentUser ? "bg-blue-500/[0.05]" : ""}`}>
      ...
      {/* Avatar + name */}
      <Link href={`/profile/${user.username}`} className="flex items-center gap-2.5 group flex-1 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-xs font-semibold text-white/80 truncate group-hover:text-blue-300 transition-colors leading-tight">
            {user.name}
            {user.isCurrentUser && (
              <span className="ml-1.5 text-[9px] font-bold text-blue-400 bg-blue-400/10 px-1.5 py-0.5 rounded-full align-middle">You</span>
            )}
          </p>
      ...
      <div className="w-10 flex justify-center items-center gap-1 shrink-0">
        <Flame size={12} className={user.streak > 0 ? "text-orange-400" : "text-white/15"} />
        <span className={`text-xs font-semibold tabular-nums ${user.streak > 0 ? "text-white/80" : "text-white/25"}`}>
          {user.streak}
        </span>
      </div>
      ...
      <div className="w-16 flex justify-center items-center gap-1 shrink-0">
        ...
        <span className="text-xs font-semibold tabular-nums text-white/60 truncate">{user.focusTime}</span>
      </div>

      {/* Completion */}
      <div className="w-12 flex justify-center items-center gap-1 shrink-0">
        <Target size={11} className={`${completionColor} shrink-0`} />
        <span className={`text-xs font-semibold tabular-nums ${completionColor}`}>
          {user.completionRate}%
        </span>
      </div>
    </div>
    ...
    <div className="lunar-card overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-4 sm:p-5 border-b border-white/[0.06]">
        <div className="flex items-center gap-3 min-w-0">
          <h2 className="lunar-page-title text-2xl sm:text-3xl truncate">Live Rankings</h2>
          {isPending && (
      ...
      <div className="flex items-center px-3 sm:px-5 py-2.5 border-b border-white/[0.06] gap-2">
        ...
        <div className="w-16 flex justify-center shrink-0">
          <span className="lunar-label !mb-0 text-[10px] sm:text-xs">Time</span>
        </div>
        <div className="w-12 flex justify-center shrink-0">
          <span className="lunar-label !mb-0 text-[10px] sm:text-xs">Done</span>
        </div>
      </div>
