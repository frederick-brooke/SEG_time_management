...
jest.mock("@/components/ui/Calendar", () => ({
	...
	Calendar: ({ selected, onSelect }: any) => (
		<div>
			<div data-testid="calendar-selected-count">
				{selected?.length ?? 0}
			</div>
			<div data-testid="calendar-first-selected-type">
			...
			<button
				onClick={() =>
					onSelect?.([new Date("2026-06-10T00:00:00.000Z")])
				}
			>
...
describe("Exam detail page [id]", () => {
	...
	it("calls generateExamPlan when Generate Study Plan is clicked", async () => {
		...
		expect(generateExamPlan).toHaveBeenCalledWith(
			"exam-123",
			expect.any(Array),
		);
	});
	...
	it("calls updateExamUnavailableDays when a calendar day is selected", async () => {
		...
		} = require("@/app/actions/examActions");
		...
		await act(async () => {
			fireEvent.click(screen.getByText("mock-calendar-select"));
		});

		expect(updateExamUnavailableDays).toHaveBeenCalledWith(
			"exam-123",
			expect.any(Array),
		);
	});
	...
	it("prevents negative signs and 'e' in the duration input", async () => {
		...
		const dashEvent = fireEvent.keyDown(durationInput, {
			key: "-",
			code: "Minus",
		});
		expect(dashEvent).toBe(false);
		const eEvent = fireEvent.keyDown(durationInput, {
			key: "e",
			code: "KeyE",
		});
	...
	it("maps unavailableDays strings into Date objects for Calendar selected prop", async () => {
		...
		expect(screen.getByTestId("calendar-selected-count")).toHaveTextContent(
			"2",
		);
		expect(
			screen.getByTestId("calendar-first-selected-type"),
		).toHaveTextContent("date-object");
	});

	it("falls back to 0 when duration input is cleared", async () => {
		(getExamById as jest.Mock).mockResolvedValue(mockExam);
	...
	it("passes an empty array to Calendar when unavailableDays is undefined", async () => {
		(getExamById as jest.Mock).mockResolvedValue({
			...mockExam,
