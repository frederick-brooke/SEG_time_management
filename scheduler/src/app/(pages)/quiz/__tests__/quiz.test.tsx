...
describe("Quiz Page", () => {
  ...
  it("updates Step 4 reminderDays and pluralisation", () => {
    ...
    expect(screen.getByText(/on the day the task is due/i)).toBeInTheDocument();
  });
  ...
  it("schedule preview reflects sessionLength × breaksPerDay", () => {
    ...
    expect(screen.getByText(/~240min effective work time/i)).toBeInTheDocument();
  });
  ...
  it("all five task-order options are rendered", () => {
    ...
    const options = ["Hard tasks first", "Easy tasks first", "Deadline first", "Shortest first", "Longest first"];
  ...
  it("summary grid reflects user changes made in previous steps", () => {
    ...
    const summaryGrid = screen.getByText("Your Preferences Summary")
  ...
  it("summary shows 'Day of' when reminderDays is 0", () => {
    ...
    fireEvent.click(screen.getByText(/Day of/i));
    const summarySection = screen.getByText("Your Preferences Summary")
      .closest("div") as HTMLElement;
  ...
  it("Back returns to the previous step", () => {
    render(<QuizPage />);
    fireEvent.click(screen.getByText(/Next/i));
    expect(screen.getByRole("heading", { name: /Breaks & Sessions/i })).toBeInTheDocument();
  ...
  it("shows 'Saving…' while submit is in progress", async () => {
    let resolveSession!: (v: unknown) => void;
    (global.fetch as jest.Mock)
      .mockReturnValueOnce(new Promise((res) => { resolveSession = res; }));
