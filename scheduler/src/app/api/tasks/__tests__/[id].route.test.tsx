...
describe("Task API route", () => {
  ...
  describe("PATCH", () => {
    ...
    it("sets all optional Iif-guarded fields when provided", async () => {
      ...
      (prisma.task.update as jest.Mock).mockResolvedValue({ ...baseTask, completed: false });
  ...
  it("sets dueDate to null when falsy value provided", async () => {
    ...
    await PATCH(mockRequest({ dueDate: null }), { params: mockParams("1") });
  ...
  it("sets scheduledDate to null when falsy value provided", async () => {
    ...
    await PATCH(mockRequest({ scheduledDate: null }), { params: mockParams("1") });
  ...
  it("awards Low priority points correctly", async () => {
    (prisma.task.findUnique as jest.Mock).mockResolvedValue({ ...baseTask, completed: false });
    ...
    const res = await PATCH(mockRequest({ status: "completed" }), { params: mockParams("1") });
    const json = await res.json();
  ...
  it("clears progress when status is set to completed", async () => {
    (prisma.task.findUnique as jest.Mock).mockResolvedValue({ ...baseTask, completed: false });
    (prisma.task.update as jest.Mock).mockResolvedValue({ ...baseTask, completed: true, priority: "Low" });

    await PATCH(mockRequest({ status: "completed" }), { params: mockParams("1") });
  ...
  it("sets status and completedAt to null when status is not completed", async () => {
    (prisma.task.findUnique as jest.Mock).mockResolvedValue({ ...baseTask, completed: true });
    (prisma.task.update as jest.Mock).mockResolvedValue({ ...baseTask, completed: false });

    await PATCH(mockRequest({ status: "todo" }), { params: mockParams("1") });
  ...
  it("sets examId to null when examId is empty string", async () => {
    ...
    await PATCH(mockRequest({ examId: "" }), { params: mockParams("1") });

    expect(prisma.task.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ examId: null, category: "General" }),
      }),
    );
  });
...
it("uses exam title from DB when linking exam", async () => {
  (prisma.task.findUnique as jest.Mock).mockResolvedValue(baseTask);
  (prisma.exam.findUnique as jest.Mock).mockResolvedValue({ id: "exam1", title: "Physics" });
  (prisma.task.update as jest.Mock).mockResolvedValue(baseTask);

  await PATCH(mockRequest({ examId: "exam1" }), { params: mockParams("1") });

  expect(prisma.task.update).toHaveBeenCalledWith(
    expect.objectContaining({
      data: expect.objectContaining({ category: "Physics", examId: "exam1" }),
    }),
  );
});
...
it("falls back to General category when linked exam has no title", async () => {
  (prisma.task.findUnique as jest.Mock).mockResolvedValue(baseTask);
  (prisma.exam.findUnique as jest.Mock).mockResolvedValue({ id: "exam1", title: null });
  (prisma.task.update as jest.Mock).mockResolvedValue(baseTask);
