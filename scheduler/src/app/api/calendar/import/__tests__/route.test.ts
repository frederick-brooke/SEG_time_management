...
describe("iCal Import API Route", () => {
	...
	describe("GET", () => {
		it("should return 401 when the user is not authenticated", async () => {
			...
			const res = await GET(
				makeRequest("GET", "http://localhost/api/calendar/import"),
			);
			...
			expect(mockPrismaEvent.findMany).toHaveBeenCalledWith(
				expect.objectContaining({
					where: expect.objectContaining({
						googleEventId: expect.objectContaining({
							startsWith: "ical:",
						}),
					}),
				}),
	...
	describe("POST", () => {
		...
		it("should return 422 when the remote URL cannot be fetched", async () => {
			...
			expect(await res.json()).toMatchObject({
				message: expect.stringContaining("Failed to fetch calendar"),
			});
		});
		...
		it("should correctly infer category from event title", async () => {
			...
			mockValidICalFetch(VALID_ICAL);
			mockParseICal.mockReturnValue([
				createMockVEvent({ summary: "CS101 Exam" }),
			]);
			mockPrismaEvent.findFirst.mockResolvedValue(null);
			mockPrismaEvent.create.mockResolvedValue({});
	...
	describe("DELETE", () => {
		...
		it("should return 500 when deleteMany throws", async () => {
			mockGetServerSession.mockResolvedValue(createSession());
			mockPrismaEvent.deleteMany.mockRejectedValue(
	...
	it("normalises duplicate slashes in the URL path", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);
		mockParseICal.mockReturnValue([createMockVEvent()]);
		mockPrismaEvent.findFirst.mockResolvedValue(null);
		...
		await POST(
			makeRequest("POST", "http://localhost/api/calendar/import", {
				url: "https://example.com//calendar//feed.ics",
			}),
		);

		expect(mockFetch).toHaveBeenCalledWith(
			"https://example.com/calendar/feed.ics",
			expect.any(Object),
		);
	});

	it("skips events whose googleEventId does not start with ical:", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockPrismaEvent.findMany.mockResolvedValue([
			{ googleEventId: "google:abc123" },
			{ googleEventId: "outlook:def456" },
		]);
	...
	it("returns 422 when parseICal throws", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);
		mockParseICal.mockImplementation(() => {
	...
	it("preserves existing fields when updating and new values are absent", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);
		...
		mockPrismaEvent.findFirst.mockResolvedValue({
			...
			exceptions: ["2024-06-02T10:00:00.000Z"],
		});
	...
	it("stores exdates and null recurrence for a non-recurring imported event", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);

		const exdate = new Date("2024-06-05T10:00:00Z");
		mockParseICal.mockReturnValue([
			createMockVEvent({
				rrule: null,
				exdates: [exdate],
		...
		expect(mockPrismaEvent.create).toHaveBeenCalledWith(
			expect.objectContaining({
				data: expect.objectContaining({
					recurrence: null,
					exceptions: [exdate.toISOString()],
				}),
			}),
		);
	});

	it("skips an imported event when the encoded URL cannot be decoded", async () => {
		mockGetServerSession.mockResolvedValue(createSession());

		// Valid shape: ical:<encoded-part>:<uid>
		// But use deliberately bad base64 characters to trigger decode failure path
		mockPrismaEvent.findMany.mockResolvedValue([
			{ googleEventId: "ical:%%%:uid-1" },
		]);
	...
	it("infers Lab category from the event title", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);
		mockParseICal.mockReturnValue([
			createMockVEvent({ summary: "Physics Lab" }),
		]);
		mockPrismaEvent.findFirst.mockResolvedValue(null);
		mockPrismaEvent.create.mockResolvedValue({});
	...
	it("skips an imported event when the decoded URL is empty", async () => {
		mockGetServerSession.mockResolvedValue(createSession());

		const encodedEmpty = Buffer.from("").toString("base64"); // ""
		mockPrismaEvent.findMany.mockResolvedValue([
			{ googleEventId: `ical:${encodedEmpty}:uid-1` },
		]);
	...
	it("infers Individual Study category from the event title", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);
		mockParseICal.mockReturnValue([
			createMockVEvent({ summary: "Revision Session" }),
		]);
		mockPrismaEvent.findFirst.mockResolvedValue(null);
		mockPrismaEvent.create.mockResolvedValue({});
	...
	it("falls back to Personal category when no keywords match", async () => {
		mockGetServerSession.mockResolvedValue(createSession());
		mockValidICalFetch(VALID_ICAL);
		mockParseICal.mockReturnValue([
			createMockVEvent({ summary: "Dinner with friends" }),
		]);
		mockPrismaEvent.findFirst.mockResolvedValue(null);
		mockPrismaEvent.create.mockResolvedValue({});
