...
import { GET, fetchQuote } from "../route";
...
describe("GET /api/quote", () => {
	...
	it("retries when quoteText is missing", async () => {
		mockFetch([
			...
			{
				ok: true,
				text: async () => JSON.stringify({ quoteText: "Valid quote" }),
			},
		]);
	...
	it("rejects quotes longer than max_length", async () => {
		...
		mockFetch([
			...
			{
				ok: true,
				text: async () => JSON.stringify({ quoteText: "Short one" }),
			},
		]);
	...
	it("does not append the key query param when fetchQuote is called without a key", async () => {
		global.fetch = jest.fn().mockResolvedValue({
			ok: true,
			text: async () => JSON.stringify({ quoteText: "Hello" }),
		});

		await fetchQuote(undefined);
