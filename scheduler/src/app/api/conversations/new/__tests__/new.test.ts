...
beforeEach(() => {
	...
	jest.mocked(prisma.conversation.create).mockResolvedValue(
...
describe("POST /api/conversations/new", () => {
	describe("authentication", () => {
		it("returns 401 when there is no session", async () => {
			jest.mocked(getServerSession).mockResolvedValue(null as any);
	...
	describe("request validation", () => {
		it("returns 400 when request JSON is invalid", async () => {
			...
			expect(data.error).toBe("Missing targetUserId");
	...
	describe("error handling", () => {
		it("returns 500 when the database throws on create", async () => {
			jest.mocked(prisma.conversation.create).mockRejectedValue(
				new Error("DB error"),
			);
