      ...
      <LunarCard className="max-w-2xl w-full p-8 rounded-[2.5em] backdrop-blur-xl bg-white/5 border border-white/10 shadow-[0_0_40px_rgba(59,130,246,0.15)]">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center text-xs text-white/50 mb-2">
            <span className="lunar-page-subtitle">Step {currentStep} of 4</span>
          ...
          <div className="flex justify-between mt-4">
            {STEP_TITLES.map((title, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                <div className={`lunar-page-subtitle w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all
                  ${i + 1 < currentStep  ? "bg-gradient-to-br from-blue-500 to-purple-500 text-white"
                  : i + 1 === currentStep ? "bg-blue-500 text-white ring-4 ring-blue-500/20 scale-110"
                  :                         "bg-white/10 text-white/30"}`}
                >
                  {i + 1 < currentStep ? "✓" : i + 1}
                </div>
                <span className={`lunar-page-subtitle text-[10px] font-medium hidden sm:block ${i + 1 === currentStep ? "text-blue-400" : "text-white/30"}`}>
                  {title}
                </span>
              </div>
            ))}
        ...
        {currentStep === 1 && (
          <div className="flex flex-col gap-5">
            <div>
              <Label className="block text-sm font-semibold text-white/70 mb-2">When do you start working?</Label>
            ...
            <div>
              <Label className="block text-sm font-semibold text-white/70 mb-2">When do you stop working?</Label>
              <Input type="time" value={formData.workEndTime}
                onChange={(e) => handleChange("workEndTime", e.target.value)}
                className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-blue-400/60 transition-colors" />
              {workingWindowLabel && (
                <p className="text-xs text-blue-400/80 mt-1.5 lunar-page-subtitle">{workingWindowLabel}</p>
              )}
            </div>

            <div>
              <Label className="block text-sm font-semibold text-white/70 mb-3">Which days are you off?</Label>
        ...
        {currentStep === 2 && (
            ...
            <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
              	<Label className="block text-sm font-semibold text-white/70 mb-3">How long are your breaks?</Label>
				<div className="flex items-center gap-3">
				...
				<div className="flex justify-between text-[10px] text-white/30 mt-1 lunar-page-subtitle">
					<span>5m</span><span>15m</span><span>30m</span><span>60m</span>
				</div>
            </div>
            ...
            <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-2xl p-4 border border-blue-400/20">
				...
				<p className="text-sm text-white/80">
					Work <strong className="text-white">{formData.sessionLength}min</strong> → Break{" "}
					<strong className="text-white">{formData.breakLength}min</strong>, repeated up to{" "}
					<strong className="text-white">{formData.breaksPerDay}×</strong> per day
				</p>
				<p className="lunar-page-subtitle text-xs text-white/40 mt-1">
					~{formData.sessionLength * formData.breaksPerDay}min effective work time per day
				</p>
            </div>
          </div>
        )}
        ...
        {currentStep === 3 && (
          <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
				<div>
					<Label className="block text-sm font-semibold text-white/70 mb-2">Max tasks per day</Label>
				...
				<div>
					<Label className="block text-sm font-semibold text-white/70 mb-2">Default task duration</Label>
					...
					<span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-white/30 lunar-page-subtitle">min</span>
					</div>
				</div>
            </div>
          </div>
          	...
          	<div className="flex flex-col gap-5">
				...
				<div className="bg-white/5 rounded-2xl p-5 border border-white/10 mt-2">
					...
					<div className="grid grid-cols-2 gap-4 text-sm">
						{[
						{ label: "Work hours",   value: `${formData.workStartTime} – ${formData.workEndTime}` },
						{ label: "Days off",     value: formData.daysOff.length > 0 ? formData.daysOff.join(", ") : "None" },
						{ label: "Session",      value: `${formData.sessionLength}min work, ${formData.breakLength}min break` },
						{ label: "Task order",   value: formData.taskOrder.replace(/-|_/g, " ") },
						{ label: "Max tasks/day",value: String(formData.maxTasksPerDay) },
						{ label: "Reminders",    value: formData.reminderDays === 0 ? "Day of" : `${formData.reminderDays}d before` },
						].map(({ label, value }) => (
						<div key={label}>
							<span className="lunar-page-subtitle text-xs text-white/30 block mb-0.5">{label}</span>
