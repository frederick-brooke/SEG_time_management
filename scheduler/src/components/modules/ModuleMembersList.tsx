 ...
 */
function MemberRow({ member, isOwner, currentUserRole, isUpdating, onRoleChange, onRemove }: MemberRowProps) {
    ...
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="flex items-center gap-1">
          {canManageRole && (
            <Button
              onClick={() => onRoleChange(member.user.id, member.role)}
              disabled={isUpdating}
            ...
            >
              {isUpdating ? "..." : member.role === 'ADMIN' ? "Remove Admin" : "Make Admin"}
            </Button>
          )}
