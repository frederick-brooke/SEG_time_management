 ...
 */
function MemberProgressBadge({ count, members, label, color }: {
  count: number;
  members: MemberUser[];
  label: string;
  color: 'green' | 'amber';
...
function MemberTaskRow({ task }: { task: ModuleTask }) {
  return (
    <div className={`flex items-center p-4 border rounded-xl transition-all ${
      task.completed ? 'bg-transparent border-white/5 opacity-60' : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20'
    }`}>
      <div className="flex items-center gap-3 flex-1">
        {task.completed
          ? <CheckCircle size={18} className="text-emerald-400 shrink-0 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]" data-testid="completed-icon" />
          : <Circle size={18} className="text-white/30 shrink-0" data-testid="incomplete-icon" />
        }
 ...
 */
function OwnerTaskRow({ task, onEdit, onDelete }: {
 ...
 * @param {{ tasks: ModuleTask[]; tasksWithProgress: TaskWithProgress[]; isOwnerOrAdmin: boolean; onEdit: (task: TaskWithProgress) => void; onDelete: (groupId: string) => void }} props
 * @return {JSX.Element} Tasks list card.
 */
export default function ModuleTasks({ tasks, tasksWithProgress, isOwnerOrAdmin, onEdit, onDelete }: {
  tasks: ModuleTask[];
          ...
          <div className="max-h-80 overflow-y-auto space-y-2 pr-1 lunar-scroll">
            {tasksWithProgress.map((task) => (
              <OwnerTaskRow
                ...
                task={task}
                onEdit={() => onEdit(task)}
                onDelete={() => onDelete(task.moduleTaskGroupId)}
              />
            ))}
          </div>
        ) : (
          <p className="lunar-value text-center py-8">No tasks assigned yet. Create one using the button above!</p>
        )
      ) : (
        tasks.length > 0 ? (
          <div className="max-h-80 overflow-y-auto space-y-2 pr-1 lunar-scroll">
          ...
          <p className="lunar-value text-center py-8">No tasks assigned to you yet.</p>
        )
      )}
    </div>
  );
