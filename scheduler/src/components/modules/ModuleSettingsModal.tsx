 ...
 */
function SettingsFormFields({ formData, currentMemberCount, onChange }: { formData: SettingsFormState; currentMemberCount: number; onChange: (updates: Partial<SettingsFormState>) => void }) {
  return (
    <>
      <div>
 ...
 */
function FormActionButtons({ onClose, isSubmitting, error }: { onClose: () => void; isSubmitting: boolean; error: string | null }) {
  return (
    <>
      {error && (
      ...
      <div className="flex gap-3 pt-4">
        <Button type="button" onClick={onClose} disabled={isSubmitting} className="flex-1 lunar-button-ghost disabled:opacity-50">
          Cancel
        </Button>
...
export default function ModuleSettingsModal({ module, onClose, onSuccess }: ModuleSettingsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setError('error' in result ? result.error as string : "Failed to update settings");
    }
  };
