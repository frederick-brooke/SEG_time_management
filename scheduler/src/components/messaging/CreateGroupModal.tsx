...
export function CreateGroupModal({
	...
	return (
		<div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60 backdrop-blur-sm">
			<div className="rounded-2xl p-6 w-full max-w-md bg-[rgba(12,16,32,0.98)] border border-white/[0.08] shadow-[0_24px_64px_rgba(0,0,0,0.6)]">
				...
				<p className="text-sm font-medium mb-2 text-[rgba(148,163,255,0.5)]">
					Add friends
				</p>
				<div className="space-y-1 max-h-52 overflow-y-auto mb-4 rounded-lg p-2 border border-white/[0.06]">
					{friends.map((u) => (
						<label
							key={u.id}
						...
						>
							...
							{resolveAvatarSrc(u.pfp) ? (
								...
								<div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium bg-[rgba(88,101,242,0.2)] text-[rgba(148,163,255,0.8)]">
									{u.username[0].toUpperCase()}
								</div>
							)}
							<span className="text-sm text-[rgba(200,210,230,0.8)]">
								{u.fname?.trim() || u.username}
							</span>
						</label>
					))}
				</div>
				...
				{/* Duplicate warning */}
				{duplicate && (
					<div className="flex items-center gap-2 rounded-lg px-3 py-2 mb-4 text-xs bg-[rgba(255,180,0,0.08)] border border-[rgba(255,180,0,0.2)] text-[rgba(255,200,80,0.9)]">
						<svg
							width="14"
							height="14"
							viewBox="0 0 24 24"
						...
						A group with these members already exists. Open it
						instead?{" "}
						<Button
				...
				<div className="flex justify-end gap-2">
					...
					<Button
						...
						disabled={
							!name.trim() || selected.length === 0 || loading
						}
