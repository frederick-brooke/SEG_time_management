...
export default function EventForm({
  ...
  const f = useEventForm(initialEvent, initialStartDate, userId, existingEvents);
  ...
  return (
    <form onSubmit={(e) => f.handleSubmit(e, onSuccess)} className="flex flex-col gap-4">
      {f.isGoogle && (
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {f.categories.map((cat: Category) => {
            const isSelected = f.category === cat.name;
            const bgClass = isSelected ? "opacity-100 scale-105" : "opacity-40";
            return (
              <button
                key={cat.id}
                type="button"
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold border border-transparent transition-all ${bgClass}`}
