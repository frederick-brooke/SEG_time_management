...
function createMarker(
  ...
  const marker = L.marker([lat, lng], { icon }).addTo(map).bindPopup(popupHtml, { maxWidth: 280 });
  return marker;
}

// Create popup HTML for an event
function buildPopupContent(event: MapEvent, color: string): string {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      events.forEach((event, index) => {
        ...
        const popupHtml = buildPopupContent(event, color);

        // Start Marker
        if (event.startCoords) {
          const [lat, lng] = offsetCoord(event.startCoords.lat, event.startCoords.lng, index, "start");
          const marker = createMarker(L, map, lat, lng, color, event.category, popupHtml);
          layersRef.current.push(marker);
        }

        // Destination Marker
        if (event.destinationCoords) {
          const [lat, lng] = offsetCoord(event.destinationCoords.lat, event.destinationCoords.lng, index, "dest");
          const marker = createMarker(L, map, lat, lng, color, "D", popupHtml);
          layersRef.current.push(marker);
