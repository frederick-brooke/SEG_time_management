		...
		>
			<div
				className={`w-4 h-4 rounded-md flex items-center justify-center flex-shrink-0 ${
					selected ? "bg-blue-500" : "border-2 border-white/20"
				}`}
			>
				{selected && (
...
export default function FutureTasksPanel({
	...
	return (
		<div className="border border-blue-500/20 rounded-2xl overflow-hidden">
			<div
				className="flex items-center justify-between p-4 bg-blue-500/10 cursor-pointer"
				onClick={() =>
					patch({ showFutureTasks: !state.showFutureTasks })
				}
			>
				<div>
					...
					<p className="text-xs text-blue-400/60 mt-0.5">
						{futureTasks.length} task
						{futureTasks.length !== 1 ? "s" : ""} beyond this period
					</p>
				</div>
				<div
					className={`w-10 h-5 rounded-full relative flex-shrink-0 transition-colors ${
						state.showFutureTasks ? "bg-blue-500" : "bg-white/10"
					}`}
				>
					<div
							...
							<Button
								...
								className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all ${
									(
										...
										: "bg-white/5 text-blue-300 border-blue-500/20 hover:border-blue-400/40"
								}`}
							>
								{m === "auto"
									? "✨ Auto-pick"
						...
						<div className="flex flex-col gap-2 max-h-44 overflow-y-auto">
							{futureTasks.map((t) => (
								<TaskRow
									key={t.id}
									task={t}
									selected={state.selectedFutureTaskIds.includes(
										t.id,
									)}
									onToggle={(id) =>
										patch({
