				...
				<div className="grid grid-cols-2 gap-3">
					<div>
						...
						<input
							...
							onChange={(e) =>
								patch({
									breakSessionMins: Number(e.target.value),
								})
							}
							className="w-full bg-white/5 border border-white/10 text-white rounded-xl p-2 text-sm focus:outline-none focus:border-blue-500 transition-colors"
						/>
					</div>
					<div>
						<p className="text-xs text-white/30 mb-1">
						...
						<input
							...
							onChange={(e) =>
								patch({
									breakLengthMins: Number(e.target.value),
								})
							}
							className="w-full bg-white/5 border border-white/10 text-white rounded-xl p-2 text-sm focus:outline-none focus:border-blue-500 transition-colors"
						/>
				...
				<div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
					<div>
						...
						<input
							...
							className="w-full bg-white/5 border border-white/10 text-white rounded-xl p-2.5 text-sm focus:outline-none focus:border-blue-300 transition-colors"
						/>
					</div>

					<div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4">
						<p className="text-[11px] font-bold text-blue-400 uppercase tracking-widest mb-2">
						...
						{weekTasks.length === 0 ? (
							...
							weekTasks.map((t) => (
								...
								>
									<div className="w-1 h-1 rounded-full bg-blue-500/50 flex-shrink-0" />
									<span className="text-xs text-blue-300 flex-1 truncate">
										{t.title}
									</span>
									<span className="text-xs text-blue-400/50 flex-shrink-0">
										{t.duration}m
									</span>
								</div>
							))
							...
							<div className="flex flex-col gap-2 max-h-44 overflow-y-auto">
								{unscheduled.map((t) => (
									<TaskRow
										...
										task={t}
										accent="blue"
										selected={state.selectedTaskIds.includes(
											t.id,
										)}
										onToggle={(id) =>
											patch({
												selectedTaskIds: toggleId(
													id,
					...
					{state.scheduleMode === "week" && (
						<div>
							...
							<div className="flex gap-2 flex-wrap">
								{allDays.map((d) => {
									const isOff =
										state.unavailableDays.includes(d);
					...
					state.missedDeadlineTasks.length === 0 && (
						<DrawerFooter className="border-t border-white/[0.06] px-6 py-4 flex-shrink-0">
							<Button
								...
								className="w-full bg-blue-300 text-gray-950 rounded-2xl py-4 text-sm font-bold hover:bg-blue-400 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
							>
								{state.isScheduling
									? "Scheduling…"
									: "Create Schedule"}
