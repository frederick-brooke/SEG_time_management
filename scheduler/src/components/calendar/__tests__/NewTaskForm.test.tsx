...
describe("NewTaskForm — rendering", () => {
	...
	it("does not show RecurrencePanel initially", () => {
		renderForm();
		expect(
			screen.queryByTestId("recurrence-panel"),
...
describe("NewTaskForm — reset after add", () => {
	...
	describe("NewTaskForm — custom date range", () => {
		beforeEach(() => jest.clearAllMocks());

		it("shows range start and end inputs when useRange is enabled", () => {
			renderForm();

			fireEvent.click(screen.getByText("Custom date"));
			fireEvent.click(screen.getByLabelText("Use a date range"));

			expect(screen.getByText("From")).toBeInTheDocument();
			expect(screen.getByText("To")).toBeInTheDocument();
		});

		it("submits custom range values when in custom range mode", () => {
			renderForm();

			fireEvent.click(screen.getByText("Custom date"));
			fireEvent.click(screen.getByLabelText("Use a date range"));

			const dateInputs = screen.getAllByDisplayValue("2026-04-01");
			...
			expect(defaultProps.onAdd).toHaveBeenCalledWith(
				expect.objectContaining({
					relativeMode: "custom",
					customDate: null,
					customRangeStart: "2026-04-10",
					customRangeEnd: "2026-04-20",
					useRange: true,
				}),
			);
		});

		it("lets the user change the single custom date", () => {
			renderForm();
			...
			fireEvent.change(screen.getByPlaceholderText("Task title"), {
				target: { value: "Single Custom Task" },
			});
	...
	// Weekly recurrence details branch
	describe("NewTaskForm — recurrence details", () => {
		beforeEach(() => jest.clearAllMocks());

		it("submits weekly recurrence with selected days and updated until", () => {
			renderForm();

			fireEvent.click(screen.getByTestId("toggle-One-time task"));

			fireEvent.click(screen.getByText("Set Weekly"));
			fireEvent.click(screen.getByText("Set Days"));
			fireEvent.click(screen.getByText("Set Until"));
		...
		it("resets priority, mode, recurrence days and specific time after add", () => {
			renderForm();

			fireEvent.change(screen.getByPlaceholderText("Task title"), {
				target: { value: "Reset Task" },
			});
			...
			expect(screen.queryByDisplayValue("13:15")).not.toBeInTheDocument();
		});
	});
	it("submits recurring task with until as null when recUntil is empty", () => {
		renderForm({ defaultUntil: "" });
