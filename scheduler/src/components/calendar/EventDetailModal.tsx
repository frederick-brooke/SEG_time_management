							...
							style={{
								backgroundColor:
									selectedEvent._type === "task"
										? TASK_COLORS[selectedEvent.priority] ||
											"#4ade80"
										: CATEGORY_COLORS[
						...
						{selectedEvent._type === "task" ? (
							<div className="flex flex-col gap-3">
								...
								{selectedEvent.scheduledRelativeTo && (
									<div className="px-3 py-2 bg-indigo-500/10 border border-indigo-500/20 rounded-lg text-xs text-indigo-300 font-medium flex items-center gap-2">
										...
										<span>
											Linked to event ·{" "}
											{selectedEvent.relativeOffsetDays ===
											0
												? "same day"
												: selectedEvent.relativeOffsetDays <
													  0
								...
								{canEdit ? (
									...
									<p className="text-xs text-white/30 text-center mt-4 border border-white/10 bg-white/5 p-3 rounded-xl">
										🔒 This task belongs to a module. If you
										are the module owner, edit it from the
										Modules page.
									</p>
								)}
							</div>
							...
							<div className="flex flex-col gap-3">
								...
								{canEdit ? (
									...
									<p className="text-xs text-white/30 text-center mt-4 border border-white/10 bg-white/5 p-3 rounded-xl">
										🔒 This event belongs to a module. If
										you are the module owner, edit it from
										the Modules page.
									</p>
								)}
							</div>
