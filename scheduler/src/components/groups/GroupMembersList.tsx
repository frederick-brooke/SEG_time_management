...
function MemberAvatar({ user }: { user: GroupUser }) {
  if (user.pfp) {
    return <img src={user.pfp} alt={user.username} className="w-full h-full object-cover" />;
  }
  return (
...
function MemberRow({ member, isOwner, isRemoving, onRemove }: MemberRowProps) {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
      <Link href={`/profile/${user.username}`} className="flex items-center gap-3 flex-1 min-w-0 hover:opacity-80 transition-opacity">
        <div className="w-10 h-10 bg-[#0a0f1d] border border-white/20 rounded-full overflow-hidden shrink-0">
          <MemberAvatar user={user} />
        </div>
        <div className="min-w-0">
          <p className="font-bold text-white truncate text-sm">{user.fname || user.username} {user.lname}</p>
 ...
 */
export default function GroupMembersList({ members, isOwner, groupId }: GroupMembersListProps) {
