/**
 * @file GroupEvents.tsx
 * @description Displays a scrollable list of upcoming group events.
 * Allows any group member to edit or delete shared events. Delegates 
 * individual event rendering to a dedicated sub-component.
 */
...
/**
 * Represents the data structure for a single group event.
 */
...
/**
 * Props for the main GroupEvents component.
 */
interface GroupEventsProps {
  events: GroupEventData[];
  onEdit: (event: GroupEventData) => void;
  onDelete: (groupId: string) => void;
}

/**
 * Props for the individual GroupEventItem component.
 */
interface GroupEventItemProps {
...
/**
 * Renders an individual group event card with edit and delete controls.
 *
 * @param {GroupEventItemProps} props - Component props.
 * @returns {JSX.Element} The rendered event card.
 */
function GroupEventItem({ event, onEdit, onDelete }: GroupEventItemProps) {
  ...
  return (
    <div className="flex items-start justify-between p-4 bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all rounded-xl gap-3">
      ...
      <div className="flex items-center gap-1 shrink-0">
        <Button onClick={() => onEdit(event)} className="p-1.5 text-white/30 hover:text-white hover:bg-white/20 rounded-lg transition-colors" title="Edit event">
          <Pencil size={14} />
        </Button>
        <Button onClick={() => onDelete(event.groupEventGroupId)} className="p-1.5 text-white/30 hover:text-red-400 hover:bg-red-500/20 rounded-lg transition-colors" title="Delete event">
          <Trash2 size={14} />
        </Button>
      </div>
    </div>
 ...
 *
 * @param {GroupEventsProps} props - Component props.
 * @returns {JSX.Element} The rendered list of group events.
 */
export default function GroupEvents({ events, onEdit, onDelete }: GroupEventsProps) {
  return (
