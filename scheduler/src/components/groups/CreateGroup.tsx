	...
	return (
		...
		>
			...
			<div className="flex-1 min-w-0">
				...
				<p
					className={`text-xs truncate ${isSelected ? "text-blue-300" : "text-white/40"}`}
				>
					@{friend.username}
				</p>
			</div>
...
export default function CreateGroup({ onClose, onSuccess }: CreateGroupProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (selectedIds.size === 0)
			return setError("Select at least one friend to add to the group");
		...
		if (!result.success)
			return setError(result.error || "Failed to create group");
	...
	return (
		<div
			className="lunar-overlay z-50 flex items-center justify-center p-4"
			onClick={onClose}
		>
			<div
				...
				>
					...
					<div className="flex gap-3 pt-2 shrink-0">
						...
						<Button
							...
							className="flex-1 px-6 py-3 rounded-2xl bg-blue-300 text-gray-950 font-semibold shadow-[0_0_30px_rgba(90,150,255,0.25)] hover:shadow-[0_0_50px_rgba(90,150,255,0.45)] transition disabled:opacity-50"
						>
