		...
		<div className="flex items-center justify-between mb-6">
			<div>
				...
				<p className="lunar-value text-xs mt-1">
					{isEditing
						? "Changes apply to all members' calendars"
...
function TextFields({
	...
	return (
		<>
			...
			<div>
				<label className="lunar-label flex items-center gap-1">
					<MapPin size={12} className="text-white/40" /> Location /
					Destination
				</label>
				<input
					type="text"
					...
					onChange={(e) =>
						onChange({ destLocationName: e.target.value })
					}
					className="lunar-input w-full p-3 mt-1"
				/>
			</div>
...
function DateTimeGrid({
	...
	return (
		<div className="grid grid-cols-2 gap-4">
			<DateTimeRow
				label="Start"
				dateVal={formData.startDate}
				timeVal={formData.startTime}
				onDateChange={(val) => onChange({ startDate: val })}
				onTimeChange={(val) => onChange({ startTime: val })}
			/>
			<DateTimeRow
				label="End"
				dateVal={formData.endDate}
				timeVal={formData.endTime}
				onDateChange={(val) => onChange({ endDate: val })}
				onTimeChange={(val) => onChange({ endTime: val })}
			/>
		</div>
	);
}
...
export default function GroupEventModal({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const result =
			isEditing && editingEvent.groupEventGroupId
				? await updateGroupEvent(
						editingEvent.groupEventGroupId,
						groupId,
						payload,
					)
