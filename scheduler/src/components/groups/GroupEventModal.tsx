/**
 * @file GroupEventModal.tsx
 * @description A modal interface for creating or editing group-wide events. 
...
/**
 * Represents an existing group event being edited.
 */
...
/**
 * Props for the GroupEventModal component.
 */
 ...
 */
function TextFields({ formData, onChange }: { formData: EventFormState; onChange: (updates: Partial<EventFormState>) => void }) {
  return (
    <>
      <div>
        <label className="lunar-label flex items-center gap-1">Event Title <span className="text-red-400">*</span></label>
        <input type="text" required placeholder="e.g. Group Study, Movie Night" value={formData.title} onChange={(e) => onChange({ title: e.target.value })} className="lunar-input w-full p-3 mt-1" />
      </div>
      <div>
        <label className="lunar-label flex items-center gap-1"><MapPin size={12} className="text-white/40" /> Location / Destination</label>
        <input type="text" placeholder="e.g. Student Union, Coffee Shop" value={formData.destLocationName} onChange={(e) => onChange({ destLocationName: e.target.value })} className="lunar-input w-full p-3 mt-1" />
      </div>
      <div>
        <label className="lunar-label">Description</label>
        <textarea rows={3} placeholder="Optional description..." value={formData.description} onChange={(e) => onChange({ description: e.target.value })} className="lunar-input w-full p-3 mt-1 resize-none" />
      </div>
    </>
  );
}
...
function DateTimeRow({ label, dateVal, timeVal, onDateChange, onTimeChange }: { label: string; dateVal: string; timeVal: string; onDateChange: (val: string) => void; onTimeChange: (val: string) => void }) {
  return (
    <div>
      ...
      <input type="date" required value={dateVal} onChange={(e) => onDateChange(e.target.value)} className="lunar-input w-full p-2 mb-2" />
      <input type="time" required value={timeVal} onChange={(e) => onTimeChange(e.target.value)} className="lunar-input w-full p-2" />
    </div>
  );
}
 ...
 * @returns {JSX.Element} Group event creation/edit modal.
 */
export default function GroupEventModal({ groupId, editingEvent, onClose, onSuccess }: GroupEventModalProps) {
  const isEditing = editingEvent !== null;

  const buildInitialState = (): EventFormState => {
    if (!editingEvent) {
      return { title: "", description: "", category: "Social", startDate: "", startTime: "", endDate: "", endTime: "", destLocationName: "" };
    }
    const start = splitDateTime(editingEvent.start);
    const end = splitDateTime(editingEvent.end);
    return {
      title: editingEvent.title, description: editingEvent.description || "", category: editingEvent.category,
      startDate: start.date, startTime: start.time, endDate: end.date, endTime: end.time, destLocationName: editingEvent.destLocationName || "",
    };
  };
    ...
    <div className="lunar-overlay z-50">
      <div className="lunar-card max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto lunar-scroll">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <TextFields formData={formData} onChange={handleChange} />
          ...
          {error && <div className="lunar-item-error px-4 py-3 rounded-xl border text-sm font-medium">{error}</div>}
          ...
          <div className="flex gap-3 pt-4 border-t lunar-divider">
            ...
            <Button type="submit" disabled={isSubmitting} className="flex-1 lunar-button-primary !text-white !bg-white/10 !border-white/20 hover:!bg-white/20 shadow-[0_0_15px_rgba(255,255,255,0.1)] disabled:opacity-50 disabled:cursor-not-allowed py-3">
