	...
	return (
		<form onSubmit={onSubmit} className="space-y-4 mb-8">
			...
			<Button
				...
				className="w-full py-3 lunar-button-primary !text-white !bg-white/10 !border-white/20 hover:!bg-white/20 shadow-[0_0_15px_rgba(255,255,255,0.1)] disabled:opacity-50"
			>
...
export default function GroupSettingsModal({
	...
	const availableFriends = friends.filter(
		(friend) =>
			!group.members.some((member) => member.userId === friend.id),
	);
	...
	return (
		<div className="lunar-overlay" onClick={onClose}>
			<div
				className="lunar-card max-w-md w-full p-6 max-h-[90vh] overflow-y-auto lunar-scroll"
				onClick={(e) => e.stopPropagation()}
			>
				<header className="flex items-center justify-between mb-6">
					<h2 className="lunar-header flex items-center gap-2">
						<Settings size={18} className="text-white/40" /> Group
						Settings
					</h2>
					<Button
						onClick={onClose}
