 ...
 */
function RemoveFriendButton({ friend, isPending, onRemoveFriend }: {
  friend: Friend;
  isPending: boolean;
  onRemoveFriend: FriendRowProps['onRemoveFriend'];
}) {
  const pendingClass = isPending ? "opacity-50 cursor-not-allowed" : "hover:bg-red-500/20";
 ...
 */
function FriendRow({ friend, isOwnProfile, onRemoveFriend, isPending }: FriendRowProps) {
  const displayName = `${friend.fname ?? friend.username} ${friend.lname ?? ""}`.trim();
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
      ...
      {isOwnProfile && (
        <RemoveFriendButton friend={friend} isPending={isPending} onRemoveFriend={onRemoveFriend} />
      )}
    </div>
  );
}
 ...
 */
export default function FriendsList({ friends, isOwnProfile, onClose, onRemoveFriend, isPending }: FriendsListProps) {
