...
function SenderAvatar({ sender }: { sender: Sender }) {
  ...
  return (
    <div className="w-10 h-10 bg-white/10 rounded-full overflow-hidden border border-white/10">
      {sender.pfp ? (
        <img src={avatarSrc} alt={sender.username} className="w-full h-full object-cover" />
      ) : (
 ...
 */
function RequestActions({ senderId, isPending, onAccept, onReject }: {
  senderId: string;
  isPending: boolean;
 ...
 */
function RequestRow({ request, isPending, onAccept, onReject }: RequestRowProps) {
  const { sender } = request;
  const displayName = `${sender.fname ?? sender.username} ${sender.lname ?? ""}`.trim();
...
export default function PendingRequests({ requests = [] }: PendingRequestsProps) {
  ...
  const handleAccept = (requestId: string) => {
    startTransition(async () => { await acceptFriendRequest(requestId); });
  };

  const handleReject = (requestId: string) => {
    startTransition(async () => { await declineFriendRequest(requestId); });
  };
