...
describe("UserFilter", () => {
	...
	test("sorting select updates sortBy and resets page", () => {
		const setFilters = jest.fn((updater) => updater(baseFilters));
	...
	test("role toggle adds category and resets page", () => {
		const setFilters = jest.fn((updater) => updater(baseFilters));
	...
	test("role toggle removes category when already selected", () => {
		const filtersWithAdmin = {
			...baseFilters,
			categories: ["SUPERUSER"],
		};

		const setFilters = jest.fn((updater) => updater(filtersWithAdmin));

		render(
			<UserFilter
				filters={filtersWithAdmin}
				setFilters={setFilters}
				onClose={onClose}
				applyFilters={applyFilters}
				resetFilters={resetFilters}
		...
		const updater = setFilters.mock.calls[0][0];
		expect(updater(filtersWithAdmin)).toEqual({
			...filtersWithAdmin,
			categories: [],
			page: 1,
		});
