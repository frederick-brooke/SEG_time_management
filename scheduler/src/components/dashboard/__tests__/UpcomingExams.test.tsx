...
describe('Upcoming Exams Module', () => {
    ...
    describe('UpcomingExams Component (UI Integration)', () => {
        ...
        it('displays the correct task count, defaulting to 0 if the tasks array is missing or empty', () => {
            ...
            expect(screen.getAllByText('0 Tasks remaining')[0]).toBeInTheDocument();
        });
        ...
        it('generates the correctly structured href paths for Next.js Links', () => {
            ...
            expect(links[0]).toHaveAttribute('href', '/exam-planner/today-exam');
            expect(links[1]).toHaveAttribute('href', '/exam-planner/upcoming-exam');
            expect(links[2]).toHaveAttribute('href', '/exam-planner/boundary-exam');
        });
    });
});