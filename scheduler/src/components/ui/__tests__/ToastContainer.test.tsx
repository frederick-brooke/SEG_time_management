...
describe("ToastItem content", () => {
	...
	it("renders the dismiss (X) button", () => {
		render(<ToastContainer toasts={[makeToast()]} onDismiss={jest.fn()} />);
...
describe("ToastItem fallback behaviour", () => {
	it("falls back to INFO styles and icon for an unknown toast type", () => {
		const weirdToast = makeToast({ type: "WHATEVER" as any });

		const { container } = render(
			<ToastContainer toasts={[weirdToast]} onDismiss={jest.fn()} />,
		);
