	...
	return (
		<div className="flex-1 min-h-0 flex flex-col">
			<GlassCard className="min-h-0 flex flex-1 flex-col p-4 overflow-hidden bg-gradient-to-b from-[#0a0a1a] via-[#1a1a3f] to-[#05051a] border-blue-300/30">
				<Header isSearching={isSearching} hasRecent={recentUsers.length > 0} onClear={() => handleClearRecent(setRecentUsers)}/>

				<div className="flex-1 flex flex-col gap-1 min-h-0 overflow-hidden">
					{!isSearching && ( <RecentUsersList users={recentUsers} setRecentUsers={setRecentUsers} />)}

					{isSearching && ( <SearchResults users={users} /> )}
				</div>

				{isSearching && users.length > 0 && (
					<Pagination filters={filters} setFilters={setFilters} start={start} end={end} totalUsers={totalUsers} totalPages={totalUserPages}/>
				)}
			</GlassCard>
...
function RecentUsersList({ users, setRecentUsers }) {
	...
	return users.map((user) => (
		<UserCard key={user.username} user={user} onClick={() => handleUserClick(user)} onRemove={() => handleRemove(user, setRecentUsers)}/>
	));
}
