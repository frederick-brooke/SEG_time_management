		...
		<section className="bg-white rounded-2xl border p-6 flex flex-col gap-5">
			...
			{workingMins > 0 && (
				<p className="text-xs text-indigo-600 font-medium">
					...
					{workingMins % 60 > 0 ? `${workingMins % 60}m` : ""} working
					window
				</p>
			)}
			<div>
				...
				<div className="flex gap-2">
					{[1, 2, 3, 4, 5, 6].map((n) => (
						<Button
							key={n}
							type="button"
							onClick={() => onChange("breaksPerDay", n)}
					...
					{TASK_ORDER_OPTIONS.map(({ value, label, desc }) => (
						...
						>
							...
							<input
								...
								onChange={(e) =>
									onChange("taskOrder", e.target.value)
								}
								aria-label={label}
							/>
							<div>
			...
			<div className="grid grid-cols-2 gap-4">
				<div>
					...
					<input
						...
						max="20"
						onChange={(e) =>
							onChange("maxTasksPerDay", parseInt(e.target.value))
						}
						className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-indigo-400"
					/>
				</div>
...
export default function PreferencesForm({
	...
	const onChange = <K extends keyof Preferences>(
		field: K,
		value: Preferences[K],
	...
	return (
		<div className="flex flex-col gap-8 max-w-2xl">
			...
			<RemindersSection
				...
				onChange={(val) => onChange("reminderDays", val)}
			/>
