...
import { type DeleteMode, type DeletableEvent } from "./undoApi";

export function useEventDelete(
  refreshEvents: () => Promise<any[]>,
  triggerUndo: (event: DeletableEvent, mode: DeleteMode) => void,
) {
  return useCallback(
    async (selectedEvent: any, mode: "single" | "series") => {
      ...
      // "single" = one instance of a recurring event (exception added on parent)
      // "series" = entire event or recurring series deleted
      triggerUndo(selectedEvent, mode === "single" ? "single" : "full");
      refreshEvents();
      return true;
    },
